...
const addToCart = async (req, res) => {
  try {
    ...
    if (!userId || !productId || quantity <= 0 || !colorId) {
      return res.status(400).json({
        success: false,
        message: "Invalid data provided!",
      });
    ...
    // Find the selected color based on the provided colorId
    ...
    // Check if the cart already has this product with the same color (_id)
    const findCurrentProductIndex = cart.items.findIndex(
      (item) =>
    ...
    // If product-color combination isn't in cart, add it; otherwise, increment the quantity
    if (findCurrentProductIndex === -1) {
      cart.items.push({ productId, quantity, colors: newColor });
    } else {
      cart.items[findCurrentProductIndex].quantity += quantity;
    }
...
const updateCartItemQty = async (req, res) => {
  try {
    ...
    if (colorId) {
      ...
      cart.items[findCurrentProductIndex].colors = selectedColor;
    }
...
const deleteCartItem = async (req, res) => {
  try {
    ...
    const populateCartItems = cart.items.map((item) => ({
      ...
      salePrice: item.productId ? item.productId.salePrice : null,
      quantity: item.quantity,
      colors: item.colors, // Include colors
    }));
